...
You come up with the following plan to keep up with his lecture and make notes.
...
The following m lines contain the words. The i-th line contains two strings a(i), b(i) meaning that the word ai belongs to the first language,
the word bi belongs to the second language, and these two words have the same meaning.
...
public class Lecture {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < lines; i++) {
            languages.put(input.next(), input.next());
        }
        String[] arr = new String[words];
        for (int i = 0; i < words; i++) {
            String temp = input.next();
            String lang2word = languages.get(temp);
            if(temp.length() > lang2word.length()){
                arr[i] = lang2word;
            }else{
                arr[i] = temp;
            }
